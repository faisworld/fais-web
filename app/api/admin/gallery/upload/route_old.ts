...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      const seoName = title.toLowerCase()
        .replace(/[^a-z0-9\s-]/g, '')
        .replace(/\s+/g, '-')
        .replace(/-+/g, '-')
        .trim();
      ...
      const insertQuery = `
        INSERT INTO images (url, title, "alt-tag", seo_name, width, height, format, size, folder, uploaded_at)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, NOW())
        RETURNING id;
      `;
      
      // Set default dimensions based on file type
      const defaultWidth = file.type.startsWith('video/') ? 1920 : 1024;
      const defaultHeight = file.type.startsWith('video/') ? 1080 : 1024;
      ...
      console.log(`Successfully inserted image with ID: ${result.rows[0].id}`);
