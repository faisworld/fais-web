...
export function GalleryWithFilter() {
    ...
    async function fetchImages() {
      try {
        ...
        const uniqueFolders = Array.from(
          new Set(validImages.filter((img: ImageItem) => img.folder).map((img: ImageItem) => img.folder)),
        ) as string[]
  ...
  const filteredImages = activeFolder === "all" ? images : images.filter((img) => img.folder === activeFolder)
      ...
      {folders.length > 0 && (
        <div className="mb-6 flex flex-wrap gap-2">
          <button
            ...
            className={`btn ${activeFolder === "all" ? "bg-gray-800" : ""}`}
          >
            All
          </button>
          {folders.map((folder) => (
            <button
              ...
              className={`btn ${activeFolder === folder ? "bg-gray-800" : ""}`}
            >
              {folder}
            </button>
          ))}
            ...
            >
              ...
              {image.folder && (
                <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <span className="text-white text-sm font-medium">{image.folder}</span>
                </div>
              )}
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80" onClick={closeModal}>
          <div className="relative max-w-[90vw] max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-2 p-3 bg-white rounded-md">
              ...
              {currentImage.folder && <p className="text-gray-500 text-sm mt-1">Folder: {currentImage.folder}</p>}
            </div>
          </div>
        </div>
      )}
